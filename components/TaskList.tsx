...
function TaskItem({ data }: { data: any }) {
  ...
  return (
    <>
      <div className={`group relative flex flex-col gap-2 rounded-xl border border-[var(--line)]/80 bg-white p-3 shadow-sm shadow-slate-900/5 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md ${
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity -mr-1 -mt-1">
            <button
              onClick={() => setIsEditOpen(true)}
              className="text-[var(--faint)] hover:text-[var(--accent)] hover:bg-[var(--accent-soft)] rounded-lg p-1.5 transition-colors"
              title="Edit task"
            >
            ...
            <button
              ...
              disabled={isPending}
              className="text-[var(--faint)] hover:text-red-500 hover:bg-red-50 rounded-lg p-1.5 transition-colors"
              title="Delete task"
            >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-1.5 rounded-lg px-1.5 py-0.5 text-[11px] font-semibold transition-colors hover:bg-[var(--surface-2)] border border-[var(--line)]/70"
            >
              {data.task.status === 'TODO' && <><Circle className="h-3 w-3 text-[var(--faint)]" /><span className="text-[var(--muted)]">To Do</span></>}
              {data.task.status === 'IN_PROGRESS' && <><PlayCircle className="h-3 w-3 text-[var(--accent)]" /><span className="text-[var(--accent)]">In Progress</span></>}
              {data.task.status === 'COMPLETED' && <><CheckCircle2 className="h-3 w-3 text-[var(--ok)]" /><span className="text-[var(--ok)]">Done</span></>}
            </button>

            {isMenuOpen && (
              <>
                <div className="fixed inset-0 z-10" onClick={() => setIsMenuOpen(false)}></div>
                <div className="absolute left-0 top-full mt-1 w-36 rounded-xl border border-[var(--line)] bg-white p-1 shadow-lg z-20">
                  <button onClick={() => setStatus('TODO')} className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-xs hover:bg-[var(--surface-2)] text-[var(--ink)]">
                    <Circle className="h-3 w-3 text-[var(--faint)]" /> To Do
                  </button>
                  <button onClick={() => setStatus('IN_PROGRESS')} className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-xs hover:bg-[var(--surface-2)] text-[var(--accent)]">
                    <PlayCircle className="h-3 w-3 text-[var(--accent)]" /> In Progress
                  </button>
                  <button onClick={() => setStatus('COMPLETED')} className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-xs hover:bg-[var(--surface-2)] text-[var(--ok)]">
                    <CheckCircle2 className="h-3 w-3 text-[var(--ok)]" /> Done
                  </button>
                </div>
              </>
            )}
          ...
          {data.category?.name && (
            <span className="inline-flex items-center rounded-md bg-[var(--accent-soft)] text-[var(--accent)] px-1.5 py-0.5 text-[10px] font-semibold truncate max-w-[120px]">
              {data.category.name}
            </span>
          )}

          {(data.topic?.name || data.project?.name) && (
            <span className="inline-flex items-center rounded-md bg-[var(--surface-2)] px-1.5 py-0.5 text-[10px] font-semibold text-[var(--muted)] truncate max-w-[120px]">
              {data.topic?.name || data.project?.name}
            </span>
          )}
        </div>
